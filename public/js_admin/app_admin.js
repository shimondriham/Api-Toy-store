import TdToys from "./tdToys.js";
import { doApiGet } from "../services/apiService.js";
import { authUser } from "../services/auth.js";

window.onload = () => {
  init();
}

const init = () => {
    authUser();
  doApi();

}

const doApi = async() => {

  let url = "https://toys1234.herokuapp.com/toys/myData";
//   let resp = await fetch(url);
//   let data = await resp.json();
  let data = await doApiGet(url);
  console.log(data);
  createTable(data);
}

const createTable = (_arJson) => {
  // מרוקן את הטבלה לפני כדי שידפיס מחדש
  document.querySelector("#id_tbody").innerHTML = ""
  _arJson.forEach((item,i) => {
    let tr = new TdToys("#id_tbody",item,i, delToys);
    tr.render()
  })
}

// פונקציה שמוחקת רשומה לפי האיי די שלה במסד
const delToys = async(_id) => {
  try {
    let urlDel = "https://toys1234.herokuapp.com/toys/"+_id;
    let resp = await fetch(urlDel, {
      method: "DELETE",
      body: JSON.stringify({}),
    //   headers: { 'content-type': "application/json" }
    headers: { 
        "x-api-key":localStorage["tok"],
        'content-type': "application/json"
     } })
    // })
    let data = await resp.json();
    // בודק אם הצליח למחוק
    // let data = await doApidel(url);
    if(data.deletedCount == 1){
      // נרצה לקרוא לגו איי פי איי שוב מחדש
      // רק לשים לב שנצטרך לאפס את הטבלה
      alert("deleted success");
      doApi();
    }

  }
  catch(err){
    console.log(err)
    alert("THere problem, come back later!");
  }
}